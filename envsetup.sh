...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cluster_") ; then
       CM_BUILD=$(echo -n $1 | sed -e 's/^cluster_//g')
    else
       CM_BUILD=
    fi
    export CM_BUILD
...
function breakfast()
{
    ...
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/cluster/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch cluster_$target-userdebug
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`sed -n -e'/ro\.cluster\.version/s/.*=//p' $OUT/system/build.prop`
        ZIPFILE=cluster-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
